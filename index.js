...
wpi = {
	...
	wiringPiISR() { console.error(new Error('not implemented yet')) },
	delay() { console.error(new Error('not implemented yet')) },
}
...
module.exports = function (homebridge) {
	...
	homebridge.registerAccessory("homebridge-gpio3", "GPIODev", DeviceAccesory);
}
...
function DeviceAccesory(log, config) {
	...
	switch (config.type) {
		case 'Lamp':
			this.device = new Lamp(this, log, config);
			break;
		case 'ContactSensor':
		case 'MotionSensor':
		case 'LeakSensor':
...
function Lamp(accesory, log, config){
	this.log = log;
	this.pin = config.pin;
